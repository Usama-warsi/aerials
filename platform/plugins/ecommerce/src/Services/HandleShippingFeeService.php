...
class HandleShippingFeeService
{
    ...
    protected function calculateDefaultFeeByAddress(?Shipping $shipping, int|float $weight, int|float $orderTotal, array $data, string $option = null): array
    {
        $result = [];
        if ($shipping) {
            ...
            $country = Arr::get($data, 'country');
            if ($rule) {
                $ruleDetail = $rule->items
                    ->where('city', $city)
                    ->where('is_enabled', 1)
                    ->first();
                $price = $ruleDetail ? ($rule->price + $ruleDetail->adjustment_price) : $rule->price;
                $result[$rule->id] = [
                    'name' => $rule->name,
                    'price' => $price,
                ];
                ...
                ->where(function (Builder $query) use ($shipping, $orderTotal, $weight) {
                    $query->where(function (Builder $sub) use ($orderTotal) {
                            $sub->where('type', ShippingRuleTypeEnum::BASED_ON_PRICE)
                        ...
                        ->orWhere(function (Builder $sub) use ($weight) {
                            $sub->where('type', ShippingRuleTypeEnum::BASED_ON_WEIGHT)
                                ->where('from', '<=', $weight)
                ...
                if (EcommerceHelper::loadCountriesStatesCitiesFromPluginLocation()) {
                    $rules = $rules->orWhere(function (Builder $query) use ($weight, $shipping, $country, $state) {
                        $query
                            ->where('shipping_id', $shipping->getKey())
                    ...
                    $rules = $rules->join('ec_shipping_rule_items', 'ec_shipping_rule_items.shipping_rule_id', '=', 'ec_shipping_rules.id')
                        ->where('ec_shipping_rule_items.country', $country)
                        ->where('ec_shipping_rule_items.state', $state);
                }
                ...
                $rules = $rules->with(['items' => function ($query) {$query->where('is_enabled', 1)->orderBy('adjustment_price');},])->get();
