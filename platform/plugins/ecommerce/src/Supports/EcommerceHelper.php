...
class EcommerceHelper
{
    ...
    public function isAvailableShipping(Collection $products): bool
    {
        ...
        $countGift = $this->countGiftProducts($products);
    
        return !($countDigital || $countGift) || $products->count() != ($countDigital + $countGift);
    }
    ...
    public function countGiftProducts(Collection $products): int
    {
        if (! $this->isEnabledSupportDigitalProducts()) {
            return 0;
        }
    
        return $products->where('product_type', ProductTypeEnum::GIFT)->count();
    }
    
    public function canCheckoutForDigitalProducts(Collection $products): bool
    {
        $digitalProducts = $this->countDigitalProducts($products);
        $giftProducts = $this->countGiftProducts($products);
        if (($digitalProducts || $giftProducts) && ! auth('customer')->check() && ! $this->allowGuestCheckoutForDigitalProducts()) {
            return false;
        }
        return true;
    }
    ...
    {
        return get_ecommerce_setting('login_option', $this->isLoginUsingPhone() ? 'phone' : 'email');
    }
    ...
    public function getGtmAttributes(string $action, Product $product, array $additional = []): string
    {
        ...
        return Html::attributes([...$attributes, ...$additional]);
    }
