    ...
    <div class="row">
        <div class="col-12 my-3 text-center">
            <div>
                ...
                <div>
                    <small
                        class="text-secondary">{{ trans('plugins/englandlogistics::englandlogistics.estimated_days', ['day' => $days]) }}</small>
                </div>
            </div>
        </div>
        <div class="col-12 my-2">
            @include('plugins/englandlogistics::address')
        </div>
                ...
                <div class="col-6">
                    @if ($cod = Arr::get($shipmentShippo, 'extra.COD'))
                        ...
                        <table class="table">
                            ...
                            <tr>
                                <td>{{ trans('plugins/englandlogistics::englandlogistics.order_amount') }}</td>
        ...
        <div class="col-12 mt-2">
            <span class="fw-bold fs-5">{{ trans('plugins/englandlogistics::englandlogistics.parcel') }}</span>
            @php
        ...
        @php
            $url = route('ecommerce.shipments.englandlogistics.transactions.create', $shipment->id);
            $isShowButton = true;
            if (!is_in_admin(true)) {
                if (is_plugin_active('marketplace')) {
                    $url = route('marketplace.vendor.orders.englandlogistics.transactions.create', $shipment->id);
                }
                ...
                <div class="col-12 my-3">
                    <div class="alert alert-warning">
                        <small>
                            ...
                            <span>{{ trans('plugins/englandlogistics::englandlogistics.note_5') }}</span>
                        </small>
                    </div>
                    <button
                        class="btn btn-primary get-new-rates"
                        data-url="{{ route('ecommerce.shipments.englandlogistics.rates', $shipment->id) }}"
                        type="button"
                    >
                        {{ trans('plugins/englandlogistics::englandlogistics.recheck_rate') }}
                    </button>
                </div>
            @endif
        @endif
