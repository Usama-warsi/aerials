...
class Englandlogistics
{
    ...
    {
        $this->liveApiToken = setting('shipping_englandlogistics_customer_id');
        $this->testApiToken = setting('shipping_englandlogistics_api_key');
        ...
        $this->packageTypes = config('plugins.englandlogistics.general.package_types', []);
        $this->serviceLevels = config('plugins.englandlogistics.general.service_levels', []);


        $this->logging = setting('shipping_englandlogistics_logging', 1);

        $this->cache = new Cache(app('cache'), self::class);

        $this->logger = Log::channel('englandlogistics');
    }
    ...
    public function canCreateTransaction(Shipment $shipment): bool
    {
        ...
        if ($order && $order->id && $order->shipping_method->getValue() == ENGLAND_SHIPPING_METHOD_NAME
