...
class EnglandlogisticsController extends BaseController
{
    ...
    public function __construct(
        ...
        protected Englandlogistics $englandlogistics
    ) {
    ...
    public function show(int $id, BaseHttpResponse $response)
    {
        ...
        try {
            $shipmentenglandlogistics = $this->englandlogistics->retrieveShipment($shipment->shipment_id);

            if ($shipmentenglandlogistics && Arr::get($shipmentenglandlogistics, 'status') == 'SUCCESS') {
                $rate = [];
                $payment = $order->payment;
                if ($payment->payment_channel->getValue() == PaymentMethodEnum::COD) {
                    $extra = Arr::get($shipmentenglandlogistics, 'extra', []);
                    $codAmount = Arr::get($extra, 'COD.amount');
                    if ($codAmount && $order->amount != $codAmount) {
                        Arr::set($shipmentenglandlogistics, 'extra.COD.amount', $order->amount);

                        $shipmentenglandlogistics = $this->refreshShipment($shipmentenglandlogistics, $order);
                        $rates = Arr::get($shipmentenglandlogistics, 'rates', []);
                        $rate = Arr::first($rates, function ($value) use ($order) {
                            return Arr::get($value, 'servicelevel.token') == $order->shipping_option;
                        });

                        if ($rate) {
                            $shipment->shipment_id = Arr::get($shipmentenglandlogistics, 'object_id');
                ...
                $content = view('plugins/englandlogistics::info', compact('rate', 'shipmentenglandlogistics', 'shipment', 'order'))->render();
            } else {
                if (Arr::has($shipmentenglandlogistics, 'message')) {
                    $errors[] = Arr::get($shipmentenglandlogistics, 'message');
                } else {
                    $errors[] = trans('plugins/englandlogistics::englandlogistics.shipment_object_id_not_found', ['id' => $shipment->shipment_id]);
                }
            }
        } catch (Throwable $th) {
    ...
    public function createTransaction(int $id, BaseHttpResponse $response)
    {
        ...
        if (! $this->englandlogistics->canCreateTransaction($shipment)) {
            abort(404);
        }

        $message = trans('plugins/englandlogistics::englandlogistics.transaction.created_success');

        $errors = [];
        $responseData = [];

        try {
            $transaction = $this->englandlogistics->createTransaction($shipment->rate_id);
            if (Arr::get($transaction, 'status') == 'SUCCESS') {
                ...
                $this->shipmentHistoryRepository->createOrUpdate([
                    ...
                    'description' => trans('plugins/englandlogistics::englandlogistics.transaction.created', [
    ...
    protected function refreshShipment(array $shipmentenglandlogistics, Order $order)
    {
        if (! Arr::has($shipmentenglandlogistics, 'extra.reference_2')) {
            Arr::set($shipmentenglandlogistics, 'extra.reference_2', $order->code);
        }

        $params = [
            'address_from' => Arr::get($shipmentenglandlogistics, 'address_from.object_id'),
            'address_to' => Arr::get($shipmentenglandlogistics, 'address_to.object_id'),
            'extra' => Arr::get($shipmentenglandlogistics, 'extra'),
            'parcels' => [Arr::get($shipmentenglandlogistics, 'parcels.0.object_id')],
        ];

        if (Arr::has($shipmentenglandlogistics, 'customs_declaration')) {
            $params['customs_declaration'] = Arr::get($shipmentenglandlogistics, 'customs_declaration');
        }

        if (Arr::has($shipmentenglandlogistics, 'metadata')) {
            $params['metadata'] = Arr::get($shipmentenglandlogistics, 'metadata');
        }

        return $this->englandlogistics->createShipment($params);
    }
    ...
    public function getRates(int $id, BaseHttpResponse $response)
    {
        ...
        try {
            ...
            $rates = $this->englandlogistics->sortRates($rates);
            ...
            $content = view('plugins/englandlogistics::rates', compact('rates', 'shipmentenglandlogistics', 'shipment', 'order', 'rate'))->render();
        } catch (Throwable $th) {
            $errors[] = $th->getMessage();
        }
    ...
    public function updateRate(int $id, Request $request, BaseHttpResponse $response)
    {
        ...
        return $response->setError((bool) $errors)
            ...
            ->setMessage($errors ? Arr::first($errors) : trans('plugins/englandlogistics::englandlogistics.updated_rate_success'));
    }
    ...
    protected function check(Shipment $shipment): bool
    {
        ...
        if (! $order
            || ! $order->id
            || $order->shipping_method->getValue() != ENGLAND_SHIPPING_METHOD_NAME
