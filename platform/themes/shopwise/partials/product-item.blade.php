@if ($product)
    <div class="product_wrap">
        ...
        @else
            @if ($product->productLabels->count())
                ...
                @foreach ($product->productLabels as $label)
                    <span class="pr_label" @if ($label->color) style="background-color: {{ $label->color }}" @endif>{{ $label->name }}</span>
                @endforeach
                </div>
            @endif
            ...
            @if ($product->front_sale_price !== $product->price)
                <div class="row">
                    <div class="pr_flash bg-success grow" dir="ltr">{{ get_sale_percentage($product->price, $product->front_sale_price) }}</div>
                </div>
            @endif
        
            <!--@if ($product->productLabels->count())-->
            <!--    @foreach ($product->productLabels as $label)-->
            <!--        <span class="pr_labels" @if ($label->color) style="background-color: {{ $label->color }}" @endif>{{ $label->name }}</span>-->
            <!--    @endforeach-->
            <!--@else-->
            <!--    @if ($product->front_sale_price !== $product->price)-->
            <!--        <div class="pr_flash bg-success grow" dir="ltr">{{ get_sale_percentage($product->price, $product->front_sale_price) }}</div>-->
        ...
        <div class="product_img">
            <a href="{{ $product->url }}">
                <img class="" width="181" height="201" src="{{ RvMedia::getImageUrl($product->image, 'medium', false, RvMedia::getDefaultImage()) }}" alt="{{ $product->name }}" loading="lazy" />
                @if (isset($product->images[1]))
                    <img class="product_hover_img "  width="181" height="201" src="{{ RvMedia::getImageUrl($product->images[1], 'medium', false, RvMedia::getDefaultImage()) }}" alt="{{ $product->name }}"  />
                @endif
            </a>
