                                            ...
                                            <tr>
                                                <td class="product-thumbnail">
                                                    <a href="{{ $product->original_product->url }}">
                                                        <img src="{{ RvMedia::getImageUrl($cartItem->options->image, 'thumb', false, RvMedia::getDefaultImage()) }}" alt="{{ $product->name }}" loading="lazy" />
                                                    </a>
                                                </td>
...
<script>
    $(document).ready(function() {
        ...
        function updateMinusButtonState(qtyInput) {
            var currentValue = parseInt(qtyInput.val());
            var minusButton = qtyInput.siblings('.minus');
            minusButton.prop('disabled', currentValue <= 1);
        }
        ...
        // Run the script after an AJAX request is complete
        $(document).ajaxComplete(function() {
            // Reinitialize state after AJAX updates
            $('.qty').each(function() {
                updateMinusButtonState($(this));
            });
        });
